...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
    ...
    #libt.sys_set_fps(constants['LIMIT_FPS'])

    key = libt.Key()
    mouse = libt.Mouse()

    game_state = GameStates.PLAYERS_TURN  # Why is this needed, test without it later
    ...
    while not libt.console_is_window_closed():
        ...
        if take_stairs and GameStates.PLAYERS_TURN:
            for entity in entities:
                if entity.stairs and entity.x == player.x and entity.y == player.y:
                    ...
                    message_log.add_message(Message('There are no stairs here.', libt.yellow))
        ...
        for player_turn_result in player_turn_results:
            ...
            coins = player_turn_result.get('coins')
            ...
            if xp:
                ...
                if leveled_up:
                    message_log.add_message(Message('Your battle skills grow stronger! You reached level {0}'.format(
                        player.level.current_level) + '!', libt.yellow))
                    previous_game_state = game_state
                    game_state = GameStates.LEVEL_UP

            if coins:
                player.fighter.add_coins(coins)
                message_log.add_message(Message('You gained {0} coins, you now have {1} total coins.'.format(coins, player.fighter.coins)))
...
def main():
    ...
    libt.console_init_root(constants['screen_width'], constants['screen_height'], constants['window_title'], fullscreen)

    con = libt.console_new(constants['screen_width'], constants['screen_height'])
    panel = libt.console_new(constants['screen_width'], constants['panel_height'])
